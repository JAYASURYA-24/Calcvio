...
class _SliderForYearMonthState extends State<SliderForYearMonth> {
  ...
  void initState() {
    ...
    _controller.addListener(() {
      ...
      if (parsed == null) {
        setState(
          () => _errorText = 'Enter ${_displayMin()} - ${_displayMax()}',
        );
        return;
      }
  ...
  void _updateControllerText({bool force = false}) {
    final value = isYearMode ? widget.value / 12 : widget.value;
    final displayValue =
        isYearMode
            ? (value % 1 == 0
  ...
  String _displayMin() =>
      isYearMode
          ? ((widget.min / 12) % 1 == 0
              ? (widget.min / 12).toInt().toString()
              : (widget.min / 12).toStringAsFixed(1))
          : widget.min.toStringAsFixed(0);

  String _displayMax() =>
      isYearMode
          ? ((widget.max / 12) % 1 == 0
              ? (widget.max / 12).toInt().toString()
              : (widget.max / 12).toStringAsFixed(1))
  ...
  String _formatValue(String input) {
    ...
    if (trimmed.isEmpty || trimmed == '.' || trimmed.endsWith('.')) {
      final value = isYearMode ? _defaultValue / 12 : _defaultValue;
      return isYearMode
          ? (value % 1 == 0
              ? value.toInt().toString()
              : value.toStringAsFixed(1))
  ...
  Widget build(BuildContext context) {
    ...
    final divisions =
        isYearMode
            ? ((widget.max / 12 - widget.min / 12) / 0.5).round()
            : (widget.max - widget.min).round();
